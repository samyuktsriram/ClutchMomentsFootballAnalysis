...
def plot(data_tuple):
    ...
    fig.supxlabel('Goal Difference at the 80th Minute')
    fig.supylabel('Goal Scoring Likelihood')
    fig.suptitle('Goal Scoring Likelihood Distributions', size = 15)
    ax[0].set_title(f'{data_tuple[2]} - {str(data_tuple[0].count()[0])} games')
    ax[1].set_title(f'{data_tuple[3]} - {str(data_tuple[1].count()[0])} games')
